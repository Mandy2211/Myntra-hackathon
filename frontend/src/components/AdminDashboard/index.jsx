...
function PendingProductCard({ product, onApprove, onReject }) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden hover:border-purple-500/40 transition group">
      ...
      <div className="p-4 space-y-3">
        <div>
          <h4 className="font-bold text-slate-200 line-clamp-2 text-sm">{product.name}</h4>
        ...
        <div className="flex justify-between items-center">
          ...
          <span className="text-xs text-slate-500 bg-slate-800 px-2 py-0.5 rounded">{product.category}</span>
        </div>
        {product.purl && (
        ...
        <div className="flex gap-2 pt-1">
          <button
            ...
            className="flex-1 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold flex items-center justify-center gap-1.5 transition disabled:opacity-50"
          >
...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans">
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 right-4 z-50 bg-slate-800 border border-slate-700 text-white px-5 py-3 rounded-xl shadow-2xl text-sm font-medium animate-bounce-in">
          {toast}
        </div>
      )}
      ...
      {warningSellerId && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-amber-900/50 rounded-2xl p-6 w-full max-w-md shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              <div className="p-2 bg-amber-950/50 rounded-xl">
                <AlertTriangle className="w-6 h-6 text-amber-400" />
              </div>
              <h3 className="text-lg font-bold text-white">Send Warning</h3>
            </div>
            <textarea
              value={warnMessage}
              ...
              rows={4}
              className="w-full bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-sm text-slate-200 placeholder:text-slate-500 focus:outline-none focus:border-amber-500 resize-none"
            />
            <div className="flex gap-3 mt-4">
              ...
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-500 text-white text-sm font-bold transition disabled:opacity-50"
              >
      ...
      {/* Header */}
      <header className="border-b border-slate-800 bg-slate-900/80 backdrop-blur sticky top-0 z-40 px-4 py-3 sm:px-8 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={fetchAll} className="p-2 border border-slate-700 rounded-lg text-slate-400 hover:text-white hover:border-slate-600 transition">
            ...
            {activeTab === 'sellers' && (
              <div className="space-y-4">
                {sellers.length === 0 ? (
                  ...
                  <div key={seller.id} className={`bg-slate-900 border rounded-2xl p-5 transition ${seller.isBlocked ? 'border-red-900/50 opacity-80' : 'border-slate-800 hover:border-purple-500/30'}`}>
                    <div className="flex flex-col md:flex-row md:items-center gap-4">
                      {/* Seller Info */}
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 flex-wrap">
                          <h4 className="font-bold text-white">{seller.businessName || seller.name}</h4>
                          {seller.isBlocked && (
                        ...
                        <p className="text-sm text-slate-400 mt-0.5">{seller.email} · {seller.city}, {seller.state}</p>
                      ...
                      <div className="flex gap-2 shrink-0">
                        {seller.isBlocked ? (
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-emerald-400 bg-emerald-950/40 border border-emerald-900/50 rounded-xl hover:bg-emerald-900/30 transition"
                          >
                    ...
                    {seller.WarningsReceived?.length > 0 && (
                      <div className="mt-4 pt-4 border-t border-slate-800 space-y-2">
                        {seller.WarningsReceived.map(w => (
                          <div key={w.id} className={`text-xs px-3 py-2 rounded-lg border ${w.type === 'BLOCK' ? 'bg-red-950/30 border-red-900/50 text-red-300' : 'bg-amber-950/30 border-amber-900/50 text-amber-300'}`}>
                            <span className="font-bold">{w.type}:</span> {w.message}
                            <span className="text-slate-600 ml-2">{new Date(w.createdAt).toLocaleDateString('en-IN')}</span>
                          </div>
                        ))}
                      </div>
                    )}
            ...
            {activeTab === 'reviews' && (
              <div className="space-y-3">
                ...
                {allReviews.length === 0 ? (
                  ...
                  <div key={review.id} className={`bg-slate-900 border rounded-xl p-4 ${review.isComplaint ? 'border-red-900/40' : 'border-slate-800'}`}>
                    <div className="flex flex-col sm:flex-row sm:items-start gap-3">
                      <div className="flex-1">
                        <div className="flex flex-wrap gap-2 items-center">
                          <span className="text-sm font-bold text-slate-200">{review.User?.name}</span>
