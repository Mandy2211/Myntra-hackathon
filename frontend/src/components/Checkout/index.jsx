        ...
        {done ? (
          <div className="p-8 text-center">
            ...
            <p className="text-sm text-slate-400 mb-1">
              Arriving in <span className="text-emerald-400 font-semibold">{eta.text}</span> · {payment === 'cod' ? 'Cash on Delivery' : 'Paid Online'}
            </p>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="flex gap-3">
                <img
                  src={img}
                  alt={product.name}
                  className="w-16 h-20 object-cover rounded-lg bg-slate-800 shrink-0"
                  onError={e => { e.target.src = 'https://placehold.co/120x160/1e293b/ec4899?text=Item' }}
                />
                <div className="min-w-0">
                  <h4 className="text-sm font-semibold text-slate-200 line-clamp-2">{product.name}</h4>
              ...
              {/* Delivery ETA */}
              <div className={`rounded-xl p-3 border flex items-center gap-3 ${eta.local ? 'bg-emerald-950/30 border-emerald-900/50' : 'bg-slate-950/50 border-slate-800'}`}>
                {eta.local ? <Store className="w-5 h-5 text-emerald-400 shrink-0" /> : <Truck className="w-5 h-5 text-slate-400 shrink-0" />}
                <div>
                  <div className="text-sm font-semibold text-slate-200">Arrives in {eta.text}</div>
                  <div className="text-xs text-slate-400">{eta.note}</div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="rounded-xl p-3 bg-slate-950/50 border border-slate-800 flex items-start gap-2.5">
                  <MapPin className="w-4 h-4 text-pink-400 mt-0.5 shrink-0" />
                  <div className="text-sm text-slate-300 leading-snug">
                    <span className="font-semibold text-slate-200">{user?.name}</span>
                    <div className="text-xs text-slate-400 line-clamp-2">{addressLine}</div>
                    {pincode && <div className="text-[11px] font-bold text-emerald-400 mt-0.5">PIN: {pincode}</div>}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-medium transition ${payment === 'cod' ? 'bg-pink-600/20 border-pink-500/60 text-pink-300' : 'bg-slate-950/50 border-slate-800 text-slate-400 hover:border-slate-700'}`}
                  >
                    <Wallet className="w-4 h-4" /> Cash on Delivery
                  </button>
                  <button
                    onClick={() => setPayment('online')}
                    className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-sm font-medium transition ${payment === 'online' ? 'bg-pink-600/20 border-pink-500/60 text-pink-300' : 'bg-slate-950/50 border-slate-800 text-slate-400 hover:border-slate-700'}`}
                  >
              ...
              {error && <div className="text-red-400 text-sm bg-red-950/30 border border-red-900/50 px-3 py-2 rounded-lg">{error}</div>}
            </div>
