    ...
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
      <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-950/50">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-slate-100">My Products</h2>
        </div>
        <div className="relative">
          <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" />
          <input type="text" placeholder="Search inventory..." className="bg-slate-900 border border-slate-700 rounded-lg pl-9 pr-4 py-2 text-xs text-slate-200 outline-none focus:border-pink-500 w-64" />
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-900/50 text-slate-400 text-[10px] uppercase tracking-wider border-b border-slate-800">
          ...
          <tbody className="divide-y divide-slate-800/50 text-sm">
            {products.length === 0 ? (
              ...
              <tr key={product.id} className="hover:bg-slate-800/20 transition-colors">
                ...
                <td className="px-6 py-4 font-medium text-slate-300">₹{product.price}</td>
                <td className="px-6 py-4">
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-wrap gap-2 text-[10px] font-semibold tracking-wide">
                    ...
                    <span className="bg-slate-800 text-purple-400 px-2 py-1 rounded border border-purple-500/20">
                      {product.occasion}
                    </span>
                    <span className="flex items-center gap-1 bg-slate-800 text-pink-400 px-2 py-1 rounded border border-pink-500/20">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex items-center justify-end gap-2">
                    <button className="p-1.5 text-slate-400 hover:text-blue-400 transition-colors rounded">
                      <Edit className="w-4 h-4" />
                    </button>
                    <button onClick={() => deleteProduct(product.id)} className="p-1.5 text-slate-400 hover:text-rose-400 transition-colors rounded">
